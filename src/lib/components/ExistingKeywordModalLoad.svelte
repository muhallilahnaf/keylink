<script>
	import {existingKW, toast} from '$lib/stores.js';
	import {
        delimiter, 
    ...
    const closeModal = () => {
        document.getElementById('existing-kw-modal-load-close').click()
    }

    const loadKW = async () => {
        const delim = document
            .querySelector('#existing-kw-modal-load input[name=existing-kw-delimiter]:checked')
            .id
            .replace('existing-kw-delimiter-', '')
        const excludeList = getTextListFromInput(
            'existing-kw-exclude-words', 
            ','
        )
        const fileInput = document.getElementById('existing-kw-load-text-file')
    ...
    const fetchKWData = async ({kwList, excludeList}) => {
        loading('#existing-kw-modal-load', true)
        ...
        loading('#existing-kw-modal-load', false)
        closeModal()
        if (data.error) {
            $toast = {
        ...
        $existingKW = data.existingKW
    }
</script>

<div 
    class="modal fade" 
    id="existing-kw-modal-load" 
    tabindex="-1" 
    aria-labelledby="label-existing-kw-modal-load" 
    aria-hidden="true"
>
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-light">
                <h1 class="modal-title fs-5" id="label-existing-kw-modal-load">
                ...
                <button 
                    ...
                    aria-label="Close"
                    id="existing-kw-modal-load-close"
                >
                </button>
            </div>
            <div class="modal-body bg-light">
                <div class="bg-tertiary mb-3 p-2">
                    ...
                    <label 
                        for="existing-kw-load-text-file" 
                        class="form-label"
                    >
                        Load Text File
                    </label>
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="form-check form-check-inline">
                                <input 
                                    ...
                                    type="radio" 
                                    id="existing-kw-delimiter-newline"
                                    name="existing-kw-delimiter"
                                    checked
                                >
                                <label 
                                    class="form-check-label" 
                                    for="existing-kw-delimiter-newline"
                                    role="button"
                                >
                                    newline (\n)
                                </label>
                            ...
                            <div class="form-check form-check-inline">
                                <input 
                                    class="form-check-input" 
                                    type="radio" 
                                    id="existing-kw-delimiter-comma"
                                    name="existing-kw-delimiter"
                                >
                                <label 
                                    class="form-check-label" 
                                    for="existing-kw-delimiter-comma"
                                    role="button"
                                >
                                    comma (,)
                                </label>
